...
│   agent-loop    循环驱动器:两态机 idle/running(v1.0)                  │
...
| **状态机** | agent-loop 状态机(v1.0 实测):idle↔running 两态;同会话仅一个 running,新输入入队;终态由纯文本(自然)或三闸(轮数/预算/超时/取消)触发。设计稿曾含 paused/stopping/terminated,均无赋值点(死态),已按 ADR-014 删除;运行时暂停/恢复归 v1.1 |
