...
def create_agent(session_id: str, spine: Any, cfg: Any) -> Agent:
    ...
    ag.ctx.compactor = getattr(spine, "compactor", None)  # F058 压缩器(请求边界触发)
    ag.ctx.goals = getattr(spine, "goals", None)  # F047 目标管理器(goal.* 工具 + sysprompt 段)
    ag.ctx.todos = getattr(spine, "todos", None)  # #32 待办管理器(todo.* 工具)
    ag.ctx.ask = getattr(spine, "ask", None)      # #38 反问宿主(user.ask 工具)
    ag.ctx.skills = getattr(spine, "skills", None)  # F073 技能管理器(skill.load + 目录段)
    ag.ctx.plan = getattr(spine, "plan", None)    # F045/F046 计划模式
    ag.ctx.schedule = getattr(spine, "schedule", None)  # F048 调度器
    ag.ctx.jobs = getattr(spine, "jobs", None)    # F051 后台任务
    ag.ctx.subagent = getattr(spine, "subagent", None)  # F049 子 Agent
    ag.ctx.search_backend = getattr(spine, "search_backend", None)  # F037
    ag.ctx.streaming = bool(getattr(spine, "streaming", False))  # F027 流式开关
    ag.ctx.budget = getattr(spine, "budget", None)  # F032 只读预算门面(仪表盘/报表)
    ag.ctx.session_query = getattr(spine, "fts", None)  # F057 索引句柄(懒激活后回填)
    ag.ctx.tool_registry = getattr(spine, "tool_registry", None)  # 工具注册表(F057 announce 注册面)
