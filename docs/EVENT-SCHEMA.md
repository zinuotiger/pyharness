...
> **阅读约定**:每事件小节固定四要素——**字段表**(`✓`=必填,`—`=可选)/ **必填校验** / **消费方** / **示例**(payload 形态,信封省略规则见 §2.1);**落盘时机**分强同步(§1.2)/ 普通(攒批 ≤0.5s 或 ≥64 条)/ 仅总线(瞬时)。E 组(阶段 4-6)事件字段按 PRD §3.3 要点 + 对应 F 展开,未实现前可先冻结(§7)。词表 **74** 个事件名按 A-F 分组(F 组为治理层事件:`policy.updated` **已注册**、余 3 型预登记,见 §3.6),另含 F028 明示落盘的 `llm.retry`。
...
## 3.6 F — 治理层侧(Governance v1.0;**预登记,未实现**)

> **状态(2026-09-14 勘误)**——`policy.updated` **已于 S2-2.1 注册**(词表 73 → **74**,并入 `SYNC_TYPES`,契约见 ADR-020),**不再属预登记**;**其余 3 型**(`decision.issued` / `receipt.emitted` / `evidence.archived`)**仍为预登记(冻结待激活)**——实现前经 `session.append` 一律 `EVT-102` 拒写,须走 §7 规则 5「词表登记制」并升词表版本(**74 → 77**)。
> **命名权威**:本组名称以治理期冻结记录 `ARCHITECTURE_DECISION_RECORD.md` §0.1 与 ADR-018「契约冻结清单」为准(即下表 4 名)。**任何早期草案中的别名(如 `decision.receipt` / `evidence.recorded` / `governance.audit`)不成立**;若见诸他处,以本表为准。
> **依据**:ADR-013(治理层独立成层) · ADR-015(决策事件双轨) · ADR-018(接口契约冻结) · `GOVERNED_AGENT_RUNTIME_DESIGN.md` §2.3。

| 事件(冻结名) | 必填字段表(字段:类型=说明) | 通道 | 消费方 |
|---|---|---|---|
| `policy.updated` | `policy_id`str;`version`str;`fingerprint`str(内容哈希);`op`enum=add\|tighten\|disable;`added`list—;`reason`str | 强同步 | 治理审计(策略版本追溯);替代 `scope.updated` 的策略语义面(`scope.updated` 保留兼容) |
| `decision.issued` | `decision_id`str;`tool`str;`verdict`enum=allow\|reject\|approval;`policy_refs`list;`guard_ids`list;`principal`dict;`inputs_digest`str | 强同步 | 治理审计因果链;与 `guard.evaluated`(规则级)经 `trace.call_id` 关联;承载 INV-G1 |
| `receipt.emitted` | `receipt_id`str;`decision_id`str;`kind`enum=decision\|approval;`digest`str;`prev_hash`str— | 强同步 | 凭证离线核验(`verify()`);承载 INV-G2 / INV-G3 |
| `evidence.archived` | `evidence_id`str;`claim`str;`refs`list(seq\|decision_id\|segment);`artifact_path`str— | 普通(攒批) | 证据链与运行时追溯矩阵;承载 INV-G4 |

**不变式(实现时须有专项测试,`tests/invariants/`)**:INV-G1 任何工具执行前必有同 `call_id` 的 `decision.issued`;INV-G2 每条 `approval.granted` 必对应一条 `receipt.emitted`;INV-G3 `receipt.verify()` 在真源重放后结果不变;INV-G4 `evidence`/`receipt` 只含引用与哈希(不存第二真源);INV-G5 治理层无执行/放行 API;INV-G6 策略单调(`fingerprint` 变化只体现新增/收紧)。
