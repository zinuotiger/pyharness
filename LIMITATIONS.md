...
| # | 限制 | 影响范围 | 计划 |
|---|---|---|---|
| **L-1** | 桌面租户隔离：**归属面已闭合，鉴权面未闭合**。归属面（R31-1）已改为 fail-closed 不变式——请求声明的租户必须**等于**会话的权威归属（认领集合 ∪ 全部权威目录的落盘 `tenant_id`），服务端**不再回落客户端头、也不再跨租户改判**；归属不可得 / 多租户同名 sid 一律 403。**仍存**：`X-PyHarness-Tenant` 头**未认证**（全局单 `_api_token`）⇒ 知道租户名并持有该 token 即可访问该租户全部会话（原正则漏 `-`/`.` 缺陷已修） | **同机跨租户**访问仍可能（服务仅绑 `127.0.0.1` 缓解）。另：信封未载 `tenant_id` 的历史会话经 HTTP 一律 403 | 归属面**已闭合**（R31-1，未提交，见上文 R31 段）；**已立项（用户授权）**：做 **per-tenant 鉴权**（唯一能真正闭合 L-1 的路径，属契约变更，须走 GATE-01）；立项落地前 L-1 维持 **PARTIALLY VERIFIED**，**不得**断言"已阻断跨租户访问" |
